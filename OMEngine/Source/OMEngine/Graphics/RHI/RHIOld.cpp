#include "pch.h"
#include "OMEngine/Graphics/RHI/RHIOld.hpp"
...
{
	RHIOld* RHIOld::GetInstance()
	{
		if (!_instance)
			_instance = new RHIOld();
		return _instance;
	}

	bool RHIOld::Initialize(HWND hwnd)
	{
		_frameIndex = 0;
	...
	bool RHIOld::InitializePipeline(HWND hwnd)
	{
	...
	void RHIOld::GetDebugInterface(unsigned int* dxgiFactoryFlags)
	{
	...
	bool RHIOld::CreateDevice(ComPtr<IDXGIFactory4> factory)
	{
		if (_useWarpDevice)
		{
	...
	void RHIOld::GetHardwareAdapter(IDXGIFactory1* factory, IDXGIAdapter1** adapter, bool requestHighPerformanceAdapter)
	{
		*adapter = nullptr;
	...
	bool RHIOld::CreateSwapChain(ComPtr<IDXGIFactory4> factory, HWND hwnd)
	{
	...
	void RHIOld::PopulateCommandList()
	{
	...
	bool RHIOld::CreateRootSignature()
	{
	...
	bool RHIOld::CreatePipelineState()
	{
	...
	bool RHIOld::CreateVertexBuffer()
	{
		// TODO: Temp
	...
	void RHIOld::WaitForPreviousFrame()
	{
	...
	std::vector<unsigned __int8> RHIOld::GenerateTextureData()
	{
